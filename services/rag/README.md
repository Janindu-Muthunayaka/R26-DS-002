...
existed in this repository**, which is why Component 3 has never run end to
end.

Two answers, both live:

1. **It indexes what it reads.** Every article the reader captures is stored
   (`source_type: "read"`), so the corpus builds itself out of use. For a
   personal reading assistant that is the more useful corpus anyway, and it
   waits on nobody.
2. **A seed corpus if one exists.** `--seed <folder>` loads `.txt` files or a
   `.jsonl` in the shape Nadee's ingest expected. If the newspaper corpus ever
   arrives it drops straight in.
