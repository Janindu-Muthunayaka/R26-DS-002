...
---------------------------------
  * `PROMPT` — the Sinhala system prompt, including its "answer only from the
    evidence" instruction and its "there is not enough information in the text
    that was read" fallback. That fallback is the grounding guarantee and it is
    hers.
  * `DETAIL_LEVEL_WORD_LIMITS`, `STYLE_CLASS_WORD_LIMITS`, `resolve_max_words`
    ...
    absent.
  * `_sinhala_purity` and the strict retry when an answer comes back
    code-switched.
  * The chunk metadata shape and the "always retrieve the current OCR page"
    rule.

`Work/Nadee/` is untouched. What changed is the machinery underneath — see
`store.py` for why chroma/langchain/sentence-transformers were replaced.
...
longer the embedding model; carrying them over to a different model would put
two meaningless tokens at the front of every text and quietly degrade
retrieval.
"""
...
# transformers into this service's environment. Importing `core.config` here
# would, which is why nothing below does.
...
def retrieve(store: VectorStore, query_text: str, retrieved_chunk_id=None,
    ...
    if remaining > 0:
        ...
        add([d for d in pool
             if d.get('metadata', {}).get('source_type') != 'ocr_current'][:remaining])
...
def run(store: VectorStore, ocr: dict, voice: dict, remember: bool = True
    ...
    # The page being read now REPLACES the previous one — one user, one page
    # at a time, which is the assumption Nadee's vectorstore.py flagged.
    if current:
    ...
    docs, reason = retrieve(store, query,
                            voice.get('retrieved_chunk_id'))
    if reason:
    ...
    return {
        ...
        # like a successful answer, and was reported as ok=True. Caught live,
        # not by a test.
