...
so the corpus builds itself out of actual use. After a week of testing the
system can answer questions about the articles it has actually read. This is
not a workaround for the missing file — for a personal reading assistant it is
the more useful corpus, and it needs nobody else to deliver anything.

**2. A seed corpus, if there is one.** Point `--seed` at a folder of `.txt`
files or a `.jsonl` in the shape Nadee's ingest expected, and it loads. If the
newspaper corpus ever materialises it drops straight in.
