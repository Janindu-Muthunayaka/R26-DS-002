...
Contract (docs/INTEGRATION_CONTRACT.md §3.2):
    ...
    POST /answer   {"ocr": <Layer 5 payload>, "voice": <Layer 0 output>}
              ->   {"intent", "answer_si", "retrieved_sources",
                    "speakable_text"}

Also:
    GET  /            a status page — a browser GET must never look like a fault
    GET  /health      is it up, is a key set, can it reach the model
    GET  /stats       what the store holds
    POST /ingest      {"folder": "..."} or {"texts": ["..."]}
    POST /forget      {"source_type": "read"} — drop what it has remembered
...
def build(store: VectorStore) -> FastAPI:
    ...
    def ingest(req: dict):
        ...
        return {'ok': not reason, 'added': added, 'skipped_as_known':
                len(records) - added, 'error': reason or None,
                'store': store.stats()}
