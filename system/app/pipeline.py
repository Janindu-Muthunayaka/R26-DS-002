...
class Pipeline:
    ...
    def run(self, image_paths, conf=YOLO_CONF,
        ...
        if single_article:
            ...
            # headline would be worse than saying nothing. When it refuses,
            # the old whole-frame title_bbox is not used either.
            title_box = None
            if lay.get('applicable'):
            ...
            # frame, so the user gets a correct read on the next capture
            # instead of a wrong one now. See core/config.py, SEGMENT_MODE.
            t['segment'] = round(time.time() - t0, 2)
            t['mode'] = 'too-far'
            return Document(
