...
phone will read it to a blind user in the same confident voice it uses for
the real article. The user has no way to tell the difference. That is a worse
failure than reading nothing, because reading nothing is honest.

**2. It destroys the measurement.** Component 2's contribution is mT5
post-OCR correction at CER 0.1197 -> 0.0757 over 217 page-disjoint sentences.
If a language model rewrites that output, the thing being measured is no
longer the model the thesis is about. `tests/test_polish.py` asserts that the
evaluation tools cannot switch this on.
...
So this layer exists, and it is built to be distrusted:
  ...
  * Four guards, below. A post-edit that fails any of them is DISCARDED and
    the mT5 text is used.
  * Every article it touches carries a warning saying so, all the way to the
    phone, so no transcript can be read as unassisted output.

`auto` deliberately does NOT run on 'unreadable' text
-----------------------------------------------------
That is the case where a repair would be most welcome and where invention is
most likely: with little real signal left, fluency is all the model has to go
on. On unreadable text the honest system says *"I could not read that, try
again"* — which is also the response that gets the user a better photograph,
the only thing that actually fixes the problem.
...
def check(original: str, candidate: str) -> tuple:
    ...
    one — which matters, because these four comparisons are the entire
    defence against a model inventing the news.
    """
...
def polish(text: str, mode: str = None) -> dict:
    ...
    `applied` False means the caller must keep its own text — either the
    layer is off, or the call failed, or a guard rejected the reply. In every
    one of those cases `text` in the result is the ORIGINAL, so a caller that
    ignores `applied` still cannot accidentally use an unvetted rewrite.
    """
    ...
    if mode == 'auto':
        ...
        if verdict == 'unreadable':
            # The deliberate refusal. See the module docstring.
