...
OUT: {'ok', 'route', 'intent', 'speakable', 'answer_si', 'sources', 'warnings'}

`speakable` is the only field the phone needs. The rule, stated once here and
repeated in core/schemas.py: **the phone speaks `speakable` whenever it is
non-empty, whatever `ok` says.** `ok` records whether an answer was actually
generated. A blind user must hear something, and "the answering service is not
available" is something. Silence is the one unacceptable outcome.

NOT EVERY QUESTION NEEDS COMPONENT 3
------------------------------------
"read that again" needs the text that is already in session and nothing else.
Routing those locally is not a stub — it is the correct implementation, it
works with no network and no API key, and it means the most common follow-up
survives a RAG outage during a viva.

Only GENERATE reaches the service.

MODES (core/config.py)
----------------------
`off`   the default. Local intents work; a question that needs generation gets
        an honest "not available" rather than a fabricated answer.
`http`  POST to RAG_URL/answer. Component 3 runs in its own process: it pins a
        langchain + chromadb + sentence-transformers stack, and its generator
        calls a hosted LLM, so it needs a key and live network.

TWO THINGS COMPONENT 3 STILL NEEDS BEFORE `http` CAN WORK, both outside this
file and both outstanding as of 27 Aug 2026:
  * `corpus/articles.jsonl` — `ingest.py` reads it and it does not exist in
    the repository. Without it the vector store cannot be built.
  * an API key for the generator, and network at demo time.
"""
...
# no API key — and therefore the part of the conversation that cannot break on
# the day. These are NOT stubs: each one is the correct implementation.
LOCAL_INTENTS = {
...
# listener who wants the middle of it should not have to sit through the
# start, so "next" walks the article in parts. 400 characters is roughly a
# paragraph and about twenty seconds of speech.
#
# NOT MEASURED — a readability choice, stated as one. `sentences()` does the
# splitting, so a part never begins mid-sentence.
PART_MAX_CHARS = 400

# --------------------------------------------------------------------------
# Spoken strings.
#
# NOT WRITTEN BY A NATIVE SPEAKER — have these checked before the viva. Two of
# the three are lifted verbatim from Component 3's own source (`pipeline.py`,
# `prompt.py`) so at least the wording the user hears on a RAG failure is the
# wording that component's author chose.
# --------------------------------------------------------------------------
...
def _text_of(document: Document) -> str:
    """The article as it was spoken. Same precedence as `_document_to_reply`
    in app/server.py — corrected body, falling back to raw OCR — so the text
    replayed on "again" is the text the user heard the first time."""
...
def _local(document, intent, cursor, warnings) -> dict:
    ...
    if intent == 'TITLE':
        ...
        if not titles:
            # Honest, and it stops being the answer the day Layer 4A lands.
    ...
    if intent == 'WARNINGS':
        ...
        # is how the listener finds out WHICH parts, on demand.
        #
        # The warnings themselves are English diagnostic sentences produced by
        # l3_segment. Speaking them to a Sinhala listener is imperfect and is
        # a known limitation, not an oversight - translating them means
        # translating strings that are also read by developers.
    ...
    # REPEAT / READ_ALOUD / REPLAY / anything else routed LOCAL: the whole
    # article, and the walk starts over.
...
def answer(document: Document, voice: dict, mode: str = None,
    ...
    # Component 3 returns {intent, answer_si, retrieved_sources, speakable_text}
    ...
    if reply.get('stub'):
        # tools/stub_services.py. Same guarantee as the voice side: an answer
        # that came from a placeholder must never read as Component 3 output.
