...
----------------------------------
Locating the headline is `l3_segment/closeup.headline_for_block()`; the
constants behind it are measured and their provenance is in core/config.py.
This file only READS the region it is given.
...
--------------------------------------------
  * **Coloured headlines are lost.** These pages print part of the headline in
    ...
    word `අකුමිකතා` is missing from every reading. Not fixed here — fixing it
    means a colour-aware threshold, and that should be measured before it is
    believed.
  * **A clipped headline reads partially**, because part of it is not in the
    frame. That is a capture problem, not an OCR one.
  * The result is NOT corrected by mT5. Component 2 is trained on body
    sentences; running headline fragments through it has not been measured,
    so it is not done. `article.title_raw` carries the OCR output and
    `article.title` the text that is spoken; today they are the same.
...
# A headline crop is small; anything bigger than this is not a headline and
...
def read_title_region(img, box, band_h: float = 0.0) -> tuple:
    ...
    scale the OCR COLLAPSED on one of three captures
    ('දිමුදු ිළ ුකී ි දු ී'), and any downscale into 40-90 px recovered it.
    """
    ...
    # Scale to the target band height. Capped at 1.0 for the same reason
    # OCR_SCALE_MAX is: upscaling cannot restore detail that was never
    # captured.
...
def extract(img, article: Article) -> Article:
    ...
    # The band height the region was found at, so the crop can be scaled the
    # way the measurement says it must be.
    band_h = 0.0
    try:
