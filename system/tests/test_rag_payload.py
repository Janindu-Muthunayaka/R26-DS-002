...
from layers.l5_assemble.payload import diff_tokens, rag_payload
...
def test_token_source_says_what_the_array_is():
    p = rag_payload(_doc(('a b', 'a c')))
    assert p['token_source'] == 'diff'
...
def test_the_diff_is_capped_and_the_cap_is_the_caller_s_choice():
    toks = diff_tokens(' '.join('w') * 0 + ' '.join(['w'] * 50),
                       ' '.join(['w'] * 50), max_words=10)
    assert len(toks) == 10


# ---- the payload ---------------------------------------------------------
def test_corrected_text_prefers_body_and_falls_back_to_raw():
    """Same precedence as `_document_to_reply`, so the RAG store and the
    spoken text never disagree about what was read."""
...
def test_articles_are_kept_in_order_with_diagnostics():
    ...
    assert [a['index'] for a in p['articles']] == [0, 1]
    assert p['corrected_text'].index('පළමු') < p['corrected_text'].index('දෙවන')
    assert 'verdict' in p['articles'][0]
