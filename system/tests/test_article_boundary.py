...
def test_the_threshold_sits_in_the_measured_gap():
    ...
    real captures. 1.6 — the previous value — misclassified every capture's
    tallest body line as a headline."""
    assert 1.70 < TITLE_MIN_LINE_RATIO < 5.91


def test_a_tall_body_line_is_not_a_headline():
    assert C.headline_bands(_page([(100, int(40 * 1.7), 100, 900)]), 40.0) == []


def test_a_real_headline_is_found():
    assert len(C.headline_bands(_page([(100, 240, 100, 900)]), 40.0)) == 1


def test_headline_is_attached_to_the_body_below_it():
    img = _page([(100, 240, 100, 900), (400, 30, 100, 900)])
    box = C.headline_for_block(img, (400, 1200), (100, 1000), 40.0)
...
def test_a_masthead_far_above_the_body_is_refused():
    """These pages carry a masthead, a page number and a section strip above
    the headline, all headline-sized. Measured, a real headline sits 36-97 px
    above its body while the masthead is 186-225 px above what follows."""
    med = 40.0
    far = int(med * TITLE_MAX_GAP_LINES) + 60
    img = _page([(100, 240, 100, 900), (100 + 240 + far, 30, 100, 900)])
    assert C.headline_for_block(img, (100 + 240 + far, 1200),
                                (100, 1000), med) is None


def test_a_headline_over_the_gutter_is_refused():
    img = _page([(100, 240, 60, 300), (400, 30, 700, 400)])
    assert C.headline_for_block(img, (400, 1200), (700, 1100), 40.0) is None


def test_side_by_side_headline_boxes_merge_into_one_row():
    """A two-column headline gives two boxes at the same height. Sorting by y
    then gives NEGATIVE gaps — measured -227, -260, -329 on real captures."""
    rows = C._merge_rows([(100, 340, 0, 500), (110, 350, 600, 1100)], 40)
    assert len(rows) == 1 and rows[0][2] == 0 and rows[0][3] == 1100


def test_two_separated_groups_do_not_merge():
    assert len(C._merge_rows([(100, 340, 0, 500), (900, 1140, 0, 500)], 40)) == 2


def test_no_headline_means_no_title_not_a_wrong_one():
    assert C.headline_for_block(_page([(400, 30, 100, 900)]),
                                (400, 1200), (100, 1000), 40.0) is None
...
def test_a_headline_splits_one_block_into_two_articles():
    """`blocks()` splits on white gaps only and says so: it "does NOT
    distinguish 'next article' from 'sub-heading'". MEASURED on the 70 real
    captures: 13 of the 48 that reached the column crop had a headline INSIDE
    it. After this split: 0."""
...
def test_a_headline_in_another_column_does_not_split_this_article():
    from layers.l3_segment.layout import split_at_headlines
    assert split_at_headlines([(100, 1000)], [(500, 600, 1400, 1900)],
                              (0, 1000), 1.0, 50.0) == [(100, 1000)]
