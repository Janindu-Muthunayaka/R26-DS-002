...
def test_expiry_and_miss_are_indistinguishable():
    ...
    s.put('j', _doc())
    c.advance(11)
    assert s.get('j') is None and s.get('never-existed') is None
...
def test_purge_reports_what_it_dropped():
    """Every screening step must say what it removed. Silent screening is how
    the `g0` null marker survived as a measurement of zero."""
    c = Clock()
    s = SessionStore(ttl_s=10, clock=c)
...
def test_the_cursor_lives_with_its_article():
    """A cursor that outlives the text it indexes into is a bug waiting for a
    demo."""
    c = Clock()
    s = SessionStore(ttl_s=10, clock=c)
    s.put('j', _doc())
    ...
    assert s.cursor('j') == 3
    c.advance(11)
    assert s.get('j') is None and s.cursor('j') == 0


def test_a_new_capture_resets_the_cursor():
    s = SessionStore()
    s.put('j', _doc())
    s.set_cursor('j', 5)
    s.put('j', _doc('new'))
