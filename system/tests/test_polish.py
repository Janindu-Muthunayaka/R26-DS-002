...
def test_a_summary_is_rejected_by_length():
    ok, why, _ = P.check(GOOD, 'කුරුණෑගල නගර සභාව ගැන ලිපියකි.')
    assert not ok


def test_padding_is_rejected():
    ok, why, _ = P.check(GOOD, GOOD + ' ' + GOOD)
    assert not ok
...
def test_an_english_answer_is_rejected():
    ok, why, _ = P.check(
        GOOD, 'This article is about the Kurunegala municipal council and '
              'various development projects discussed at a recent meeting of')
    assert not ok
...
def test_word_inflation_is_rejected():
    ok, why, _ = P.check(GOOD, GOOD[:len(GOOD) // 2] + ' ' +
                         ' '.join(['නව'] * 60))
    assert not ok


# ---- the network path, with a fake model ---------------------------------
...
def test_auto_refuses_unreadable_text(monkeypatch):
    ...
    fluency is all the model has to go on. The honest answer is "take another
    photograph", which is also the only thing that fixes it."""
    monkeypatch.setattr(P.llm, 'available',
                        lambda: pytest.fail('must not call the model'))
...
def test_no_evaluation_tool_can_switch_this_on():
    ...
    for name in EVAL_TOOLS:
        ...
        assert 'polish' not in src, f'{name} references the post-editor'
...
def test_the_polished_text_lives_in_its_own_field():
    """`body` is the research artifact. Nothing may overwrite it."""
