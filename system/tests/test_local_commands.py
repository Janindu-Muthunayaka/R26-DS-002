...
no API key and no teammate's component, and therefore is the part of the
conversation that cannot break during a viva. That is worth a test file of its
own.
"""
...
def test_a_short_article_is_one_part():
    assert len(l6gen._parts(_doc('කෙටි ලිපියකි.'))) == 1
...
def test_title_is_honest_while_layer_4a_is_a_stub():
...
def test_warnings_tell_the_listener_what_was_skipped():
    ...
    r = _ask(d, 'මොනවද මඟ හැරුණේ')
    assert 'off the right edge' in r['speakable']
