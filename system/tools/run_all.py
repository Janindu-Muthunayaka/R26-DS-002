...
Start the whole system: the reader, plus whichever components are ready.
    ...
    python tools\run_all.py --polish auto       # and enable Layer 4C

Ctrl-C stops everything. Each child keeps its own console output so a failure
is attributed to the process that had it.
...
worth paying at a viva, with three command windows to start in the right order
...
def spawn(label, args, cwd=None, extra_env=None):
    ...
    p = subprocess.Popen([str(a) for a in args], cwd=str(cwd or _SYSTEM), env=e)
...
def main():
    ...
    ap.add_argument('--title', choices=('stub', 'mat'), default=None,
                    help='Layer 4A. "mat" reads the headline with sin_raw.')
    a = ap.parse_args()
    ...
    print(f'  title   : {a.title or os.getenv("SINHALA_TITLE_MODE", "stub")}')
    print()
    ...
    if a.title:
        reader_env['SINHALA_TITLE_MODE'] = a.title
    ...
    try:
        if a.stubs:
            ...
            if not ok:
                ...
                reader_env['SINHALA_RAG_MODE'] = 'http'
                reader_env['SINHALA_RAG_URL'] = f'http://127.0.0.1:{a.rag_port}'
                time.sleep(2)
