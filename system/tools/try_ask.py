...
End-to-end check of the conversation path, without a phone.
    ...
    # read a page, then ask the standard set of questions
    python tools\try_ask.py --frames work\a1b2c3d4

    # ask something specific about an article already in session
    python tools\try_ask.py --job a1b2c3d4 --ask "මේක සාරාංශ කරන්න"

    # just check what the server is holding
    python tools\try_ask.py --job a1b2c3d4 --session

`--frames` takes a folder or a list of files. A folder is uploaded in sorted
order, which is the order `f0/f1/f2` were written, so frame order is the same
one the phone produced.
...
Stdlib only - urllib and a hand-rolled multipart body - so this runs in the
same environment as the server with nothing extra installed.
"""
...
# The literal words the built-in voice stub routes on (layers/l0_voice).
# Anything else goes to GENERATE and needs Component 3.
DEFAULT_QUESTIONS = [
...
def show_ask(question, gloss, reply, code):
    ...
    print(f'     route={reply.get("route", "?"):9} '
          f'intent={reply.get("intent", "?"):10} ok={ok}')
...
def main():
    ...
    if a.frames:
        ...
        print(f'  title: {_clip(reply.get("title")) or "(empty — Layer 4A is a stub)"}')
