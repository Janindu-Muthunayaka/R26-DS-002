...
`gpt-5.4-mini`; whether a given key can reach that, or anything else, is not
something source code can know. This asks.

Nothing here prints the key. `core.env.redact` shows seven characters and the
last four, which is enough to tell two keys apart in a log and useless to
anyone who reads it.
"""
...
def main():
    ...
    if llm.chat_model():
        text, reason = llm.chat(
            [{'role': 'user',
              'content': 'Reply with exactly: ok'}], max_tokens=8)
