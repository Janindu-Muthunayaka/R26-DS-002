...
RUN THIS ON WINDOWS. It needs `ultralytics`, which is in the system venv but
could not be installed in the environment this file was written in — so the
YOLO question has NOT been settled by measurement, only by a note in
`Corrections_Register.md` entry 1 from a single frame.

WHAT IT ANSWERS. For every real capture in `system/work`, it reports what the
detector returns and — the part that matters — whether its box AGREES with the
article the layout path chose. Three outcomes:

  agree     the detector found the same story. It could be trusted here.
  disagree  it found a different one. This is the failure entry 1 recorded:
            one confident box over the NEIGHBOURING article's headline, and
            because a box WAS returned the whole-frame fallback never fired.
  nothing   no box above the confidence threshold.

WHY IT MATTERS NOW. After 27 Aug the layout path handles 81% of real captures
and YOLO is the fallback for 16%. If this shows the detector agreeing on most
of that 16%, it is worth keeping as the fallback. If it disagrees, the honest
fallback is to tell the user to move closer rather than to read the wrong
story confidently — and that is a one-line change.
...
def main():
    ...
    print('\nIf DISAGREE dominates on the frames layout refuses, the detector '
          ...
          'closer", not a confident\nwrong story. That is a one-line change '
          'in app/pipeline.py.')
