...
either component is wrapped.
    ...
    python tools/stub_services.py --role rag   --port 8102

Then point the reader at them:

    set SINHALA_VOICE_MODE=http
    set SINHALA_RAG_MODE=http
    python -m app.server --root "E:/RP/corpus/Sinhala_OCR_Correction_v2"

WHAT THIS IS FOR. The `http` branch in `layers/l0_voice/voice.py` and
`layers/l6_generator/generate.py` is the branch that will run at the viva and
the branch that has no way to be tested until two other people deliver. This
lets it be tested today: the request shape, the reply parsing, the timeout,
and the degradation when a service answers badly.

WHAT THIS IS NOT. It does not detect intent and it does not retrieve anything.
Every reply it sends is marked `"stub": true`. Nothing measured against it is
a result about Component 3 or Component 4.

Stdlib only — `http.server`, no FastAPI — so it starts instantly and cannot
drag a dependency into this system's pinned environment.
"""
...
# tools/ scripts run from anywhere; put system/ on the path as they all do.
...
# duplicating that here would let the two drift apart and make a demo behave
# differently depending on which stub was in play.
...
# Sinhala for "this is a stub answer, the retrieval component is not running".
...
def make_handler(role: str, echo: bool):
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            looks like a fault and is not one: the endpoint is POST-only and
            the reader only ever POSTs to it.

            So GET answers with what the service IS, and how to actually
            exercise it. A liveness check that reads as an error is a bad
            liveness check."""
        ...
        def do_POST(self):
            ...
            if echo:
                print(json.dumps(req, ensure_ascii=False)[:600])
