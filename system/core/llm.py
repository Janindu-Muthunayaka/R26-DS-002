...
IT NEVER RAISES
---------------
Every caller is on the path to a blind user's ear or to a server that must
keep answering. A missing key, a rate limit, an outage, a model name this
account cannot reach — each returns `(None, reason)`, never a traceback.

NO MODEL NAME IS GUESSED HERE
-----------------------------
`OPENAI_CHAT_MODEL` and `OPENAI_EMBED_MODEL` come from `.env`. Guessing a
model that this key cannot reach produces a 404 at the worst possible moment,
and the model available on any given account is not something this file can
know. `python tools\\check_llm.py` lists what the key can actually see.

PARAMETER FALLBACK. Model families disagree about `temperature` and about
`max_tokens` vs `max_completion_tokens`. Rather than encode a guess about
which family is in use, a 400 that names a parameter is retried once without
it. That is defensive, not clever: it turns a hard failure into a working
call, and logs what it dropped.
"""
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        try:
            ...
            try:
                ...
                pass
            # 400 is never retried: the request itself is wrong, and sending
            # it twice more only spends time and money.
            if e.code == 400:
                return None, f'HTTP 400: {_msg(body)}'
            if e.code in (401, 403):
                return None, (f'HTTP {e.code}: the key was rejected — '
                              f'{_msg(body)}')
            if e.code == 404:
...
def _offending_param(reason: str, sent: dict) -> Optional[str]:
    ...
    was actually sent, so a stray quoted word in a message cannot make this
    drop something unrelated."""
...
def chat(messages, model: str = None, temperature: float = 0.0,
    ...
    # One retry without whatever the API objected to. Model families disagree
    # about these and this file refuses to guess which family is in play.
    if reply is None and reason.startswith('HTTP 400'):
        ...
        if drop:
            ...
            if reply is not None:
                ...
                reason = f'{reason} | after dropping {drop}: {reason2}'
...
def embed(texts, model: str = None) -> Tuple[Optional[list], str]:
    ...
    Order is asserted, not assumed: the API returns an `index` per item and
    the list is sorted by it before the vectors are handed back. A silently
    reordered batch would attach every chunk to the wrong text, and the only
    symptom would be retrieval that is subtly, unfalsifiably wrong.
    """
